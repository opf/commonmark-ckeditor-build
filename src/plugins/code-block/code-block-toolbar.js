...
export default class CodeBlockToolbar extends Plugin {
	...
	init() {
		...
		createToolbarEditButton( editor, 'opEditCodeBlock', widget => {
			...
			const language = widget.getAttribute('language');
			const content = widget.getAttribute('content');

			macroService
				.editCodeBlock( content, language )
				.then((update) => model.change(writer => {
					writer.setAttribute( 'language', update.languageClass, widget );
					writer.setAttribute( 'content', update.content, widget );
				})
			);
		} );
	}
