	...
	describe('escaping', () => {
		describe('toView', () => {
			...
			for (const key in testCases) {
				...
				it(`should escape ${key}`, () => {
					...
					expect(stringify(documentFragment)).toEqual(`<p>${result}</p>`);
				});

				it(`should not escape ${key} in code blocks`, () => {
					const documentFragment = dataProcessor.toView(`	${test}`);

					expect(stringify(documentFragment)).toEqual(`<pre><code>${test}</code></pre>`);
				});

				it(`should not escape ${key} in code spans`, () => {
					const documentFragment = dataProcessor.toView('`' + test + '`');

					expect(stringify(documentFragment)).toEqual(`<p><code>${test}</code></p>`);
				});
			}
			...
			it('should escape backtick', () => {
				...
				expect(stringify(documentFragment)).toEqual('<p>`</p>');
			});

			it('should not escape backtick in code blocks', () => {
				const documentFragment = dataProcessor.toView('	\\`');

				expect(stringify(documentFragment)).toEqual('<pre><code>\\`</code></pre>');
			});
		});
		...
		describe('HTML', () => {
			...
			it('should escape HTML as text', () => {
				testDataProcessor('\\<h1>Test\\</h1>', '<p><h1>Test</h1></p>', '&lt;h1&gt;Test&lt;/h1&gt;');
			});
